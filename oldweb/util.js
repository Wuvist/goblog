...
function d(link){
	ajaxing=1;
	document.getElementById("_rr" + link).innerHTML="评论删除中...";
	var xmlObj = null;
	...
	xmlObj.onreadystatechange = function(){
		if(xmlObj.readyState == 4){
			_updateD(link, xmlObj.responseText);
		}
	}
	xmlObj.open ('GET', "/delcomment.aspx?cid=" + link + "&t=" + timestamp(), true);
	xmlObj.send ('');
}
function timestamp()
{
	var now = new Date();
	var hours = now.getYear()+ "_" + now.getMonth() +"_"+ now.getDate()+ "_" + now.getHours() + "_" + now.getMinutes() + "_" + now.getSeconds();
	return hours;
}
function _updateD(link, data){
	var i;
	link=String(link);
	i=Number(data);
	...
	if(i==0)
	{
		document.getElementById("_c" + link).innerHTML="";
		document.getElementById("_c" + link).style.display="none";//.parentElement.removeChild("_c" + link);
	}
	else if(i==1)
	{
		alert("不好意思，请不要删别人网志的评论哈～");
	}
	else if(i==2)
	{
		alert("不好意思，你需要先登陆哈～");
...
function s(link) {
	if(isblogger==1)
	{
		if(document.getElementById("_rr" + link)==null)
		{
			var frame = document.createElement("div");
			frame.id = "_rr" + link;
			frame.style.display = "inline";
			frame.innerHTML="　<a href='javascript:d(\"" + link + "\");'>删除此评论</a> ";
			 document.getElementById("_r" + link).appendChild(frame);
		}
		else
		{
			if (document.getElementById("_rr" + link).innerHTML!="评论删除中...")
			{
				document.getElementById("_rr" + link).style.display="inline";
				//document.getElementById("_rr" + link).innerHTML="　<a href='http://www.f.com'>删除此评论</a> ";
			}			
		}

		document.getElementById("_c" + link).style.background="#f7efde";
	}
}

function s_color(link,color) {
	if(isblogger==1)
	{
		if(document.getElementById("_rr" + link)==null)
		{
		...
		else
		{
			...
			{
				document.getElementById("_rr" + link).style.display="inline";
				//document.getElementById("_rr" + link).innerHTML="　<a href='javascript:d(" + link + ")'>删除此评论</a> ";
			}			
		}

		document.getElementById("_c" + link).style.background=color;		
	}
}
function h(link) {
	if (isblogger==1&&ajaxing!=1)
	{
		document.getElementById("_rr" + link).style.display="none";
		document.getElementById("_c" + link).style.background="";
	}
}
...
function tagging() {
	...
	if (d!=null)
	{
		...
		if(isblogger==1)
		{
			...
			{
				s+= " <div id='tag" + ts[i] + "' style='display:inline' onmouseover=\"sTag('" + ts[i] + "')\" onmouseout=\"hTag('" + ts[i] + "')\"><a href='/tag.aspx?blogger=" + uid + "&tag=" + escape(ts[i]) + "'>" + ts[i] + "</a>"
				s+= " <div id='tagX" + ts[i] + "' style='display:none'>";
				s+="<a href=\"javascript:rTag('" + ts[i] + "')\" title='删除此标签，点了就删，没得反悔哦～'>X</a> </div> </div>|";
			}
			...
			dd.innerHTML="<a href='/write.aspx?edit=" + aid + "'>修改此网志</a><br /><a href='javascript:nTag()' title='点我可以添加标签哦～'>标签</a>：" + s;	
		}
		else
		{
			for(i=1;i<ts.length;i++)
			{
				s+= " <a href='/tag.aspx?blogger=" + uid + "&tag=" + escape(ts[i]) + "'>" + ts[i] + "</a> |";
			}
			s = s.substring(0,s.length-1);
			if (s=="")
			{
...
function sTag(tag)
{
	document.getElementById("tagX" + tag).style.display="inline";
}

function hTag(tag)
{
	document.getElementById("tagX" + tag).style.display="none";
}
function nTag()
{
	tagTmp=document.getElementById("tags").innerHTML;
	document.getElementById("tags").innerHTML="<input type='text' class='loginput' id='newTag' name='newTag' maxlength=49> <a href='javascript:saveTag()'>添加标签</a> <a href='javascript:cTag()'>取消</a>";
}
function cTag(s)
{
	document.getElementById("tags").innerHTML=tagTmp;
}
function saveTag(s)
{
	var ntag;
	ntag=document.getElementById("newTag").value;
	if (ntag=="")
	{
		alert("嗯？您似乎没有输入新标签");
	}
...
function ajaxTag(link,action){
	...
	document.getElementById("tags").innerHTML="程序处理中...";
...
function _updateT(data){
	var i;
	i=Number(data.substring(0,1));

	if(i==0)
	{
		document.getElementById("tags").innerHTML=data.substring(1,data.length-1);
		tagging();
	}
	else if(i==1)
	{
		alert("不好意思，不是你的标签不要乱动哈～");
		document.getElementById("tags").innerHTML=tagTmp;
	}
	else if(i==2)
	{
...
//		document.getElementById("tags").innerHTML=data;
		alert("貌似程序出错了……请按F5刷新页面重来……不过，也可以把下面的东西发给问天，你知道是谁吧？");
		document.write(data);
//		document.refresh();
	}
}
