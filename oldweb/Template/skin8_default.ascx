...
</div><p><%#Container.DataItem("content")%> </p>
<div class="datetimefooter">
posted at 
<a href="/<%# uid & "/" & Container.DataItem("index")%>.shtml">
<%# Container.DataItem("add_date") %></a> by  
<%=nick%> <a href="/<%# uid & "/" & Container.DataItem("index")%>.shtml">评论:<%#Container.DataItem("comment")%></a></div><br><br>
	</ItemTemplate>
