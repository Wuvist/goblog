...
<asp:Repeater id="archive" runat="server">
	...
	<p class=head><%=ph.g("累计浏览")%>：<%=count%></p><Acme:Message runat="server" ID="Message1"/></div>
      ...
      <div class="blog" style="width: 500; height: 403">
	  <asp:Repeater id="dlArticles" runat="server">
		<ItemTemplate>
				<a href="/<%# uid & "/" & Container.DataItem("index")%>.shtml"><div class="date"><font size="3"><b><%# Container.DataItem("title") %></b></font> </div></a>
			   <div align="right"><%=nick%> @<%# Container.DataItem("add_date") %>  
			<a href="/<%# uid & "/" & Container.DataItem("index")%>.shtml">评论:<%#Container.DataItem("comment")%></a></div><Hr>
		</ItemTemplate>
	</asp:Repeater>
      </div>
      </td>
